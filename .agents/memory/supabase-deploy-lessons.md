---
name: Supabase CLI + Postgres lessons (Fleet Arena)
...
- **Edge Function deploys**: local Docker bundling fails (package firewall DNS). Use `pnpm dlx supabase functions deploy <fn> --use-api` from `artifacts/fleet-arena/`, and delete `supabase/functions/deno.lock` if present. The API bundler ignores `deno.json` import maps — all imports must use explicit specifiers like `npm:@supabase/supabase-js@2`, `npm:zod@3.23.8`.
