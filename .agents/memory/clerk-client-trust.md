---
name: Clerk Client Trust & native redirect
description: Why native sign-in returned needs_client_trust / redirect-mismatch and how both were fixed
---
...
**Native SSO redirect (Aug 2026):** standalone builds send `navixa://` as OAuth redirect; Clerk rejects it unless allowlisted. Fix is NOT code or BAPI gymnastics — with `dashboardAccess: authorized`, the user adds it in Clerk dashboard → Configure → Developers → Native applications → "Allowlist for mobile SSO redirect" (per environment; prod fixed, dev may still lack it). Server-side only: no new EAS build needed. Also: the workspace `CLERK_SECRET_KEY` secret was invalid against api.clerk.com — always `checkClerkManagementStatus` and prefer the dashboard route before token-endpoint workarounds.
